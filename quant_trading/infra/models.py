...
class TradeCommand:
    ...
    属性:
        ...
        atr: ATR值（开仓/加仓时需要）
        reason: 操作原因
        turtle_entry_system: 系统类型 'S1' 或 'S2'
    ...
    atr: float = 0.0
    reason: str = ""
    turtle_entry_system: Optional[str] = None
    source: str = ""
...
class StrategyContext:
    ...
    由策略层构建，传递给 infra 层做存储。
    infra 层不解析其中内容，仅原样保存到DB。
    """
    entry_system: Optional[str] = None    # 入场系统（如 'S1'/'S2'）
    shares_per_unit: int = 0              # 每单位股数（已计算）
    stop_price: float = 0.0               # 止损价（已计算）
    next_add_price: float = 0.0           # 下次加仓价（已计算）
