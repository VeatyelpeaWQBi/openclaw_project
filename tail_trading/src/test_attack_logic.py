...
def test_is_attack_sector():
    ...
    for name in defense_names:
        result = is_attack_sector(name)
        print(f"  {'✓' if not result else '✗'} {name}: {'非进攻型' if not result else '进攻型(异常)'}")
    for name in neutral_names:
        result = is_attack_sector(name)
        print(f"  - {name}: {'进攻型' if result else '非进攻型'}")
...
def test_attack_filter_logic():
    ...
    print(f"  场景1 - 前10进攻型: {[s['name'] for s in top10_attack]}")
    print(f"  场景1 - 前5进攻型: {[s['name'] for s in top5_attack]}")
    ...
    top5_attack = [s for s in top5 if is_attack_sector(s['name'])]
    print(f"  场景2 - 前10进攻型: {[s['name'] for s in top10_attack]}")
    print(f"  场景2 - 前5进攻型: {[s['name'] for s in top5_attack]}")
    assert len(top10_attack) > 0, "场景2前10应该有进攻型"
    assert len(top5_attack) == 0, "场景2前5应该无进攻型"
    print("  ✓ 场景2: 前5无进攻型，应跳过")
    ...
    top10_attack = [s for s in top10 if is_attack_sector(s['name'])]
    print(f"  场景3 - 前10进攻型: {[s['name'] for s in top10_attack]}")
    assert len(top10_attack) == 0, "场景3前10应该无进攻型"
    print("  ✓ 场景3: 前10无进攻型，应跳过")
